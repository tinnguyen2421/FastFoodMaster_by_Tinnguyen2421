...
public class ShowNotification {
    ...
    public static void ShowNotif(Context context, String title, String message, String page) {
        ...
        if (page.trim().equalsIgnoreCase("Order")) {
            acIntent = new Intent(context, ChefBottomNavigation.class).putExtra("PAGE", "Orderpage");
        ...
        if (page.trim().equalsIgnoreCase("Home")) {
            acIntent = new Intent(context, CustomerBottomNavigation.class).putExtra("PAGE", "Homepage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("Confirm")) {
            acIntent = new Intent(context, ChefBottomNavigation.class).putExtra("PAGE", "Confirmpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("Preparing")) {
            acIntent = new Intent(context, CustomerBottomNavigation.class).putExtra("PAGE", "Preparingpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("Prepared")) {
            acIntent = new Intent(context, CustomerBottomNavigation.class).putExtra("PAGE", "Preparedpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("DeliveryOrder")) {
            //acIntent = new Intent(context, DeliveryBottomNavigation.class).putExtra("PAGE", "DeliveryOrderpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("DeliverOrder")) {
            acIntent = new Intent(context, CustomerBottomNavigation.class).putExtra("PAGE", "DeliverOrderpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("AcceptOrder")) {
            acIntent = new Intent(context, ChefBottomNavigation.class).putExtra("PAGE", "AcceptOrderpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        ...
        if (page.trim().equalsIgnoreCase("ThankYou")) {
            acIntent = new Intent(context, CustomerBottomNavigation.class).putExtra("PAGE", "ThankYoupage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
        if (page.trim().equalsIgnoreCase("Delivered")) {
            acIntent = new Intent(context, ChefBottomNavigation.class).putExtra("PAGE", "Deliveredpage");
            pendingIntent = PendingIntent.getActivity(context, 0, acIntent, PendingIntent.FLAG_ONE_SHOT);
        }
